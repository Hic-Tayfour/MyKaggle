...
# 5. Especificação do modelo K-NN
knn_model <- nearest_neighbor() %>%
  set_engine("kknn") %>%
  set_mode("classification") %>%
  set_args(neighbors = 5)  # Inicialmente definimos o número de vizinhos como 5

# 6. Workflow: combinar a receita e o modelo K-NN
titanic_workflow <- workflow() %>%
  add_recipe(titanic_recipe) %>%
  add_model(knn_model)

# 7. Ajustar o modelo ao conjunto de treino
fit_knn <- fit(titanic_workflow, data = train_data_split)

# 8. Fazer previsões no conjunto de validação
valid_class_preds <- predict(fit_knn, new_data = valid_data_split) %>%
  bind_cols(valid_data_split)

# Previsões de probabilidade para a Curva ROC
valid_prob_preds <- predict(fit_knn, new_data = valid_data_split, type = "prob") %>%
  bind_cols(valid_data_split)
...
ggplot(roc_curve_data, aes(x = 1 - specificity, y = sensitivity)) +
  ...
  labs(title = "Curva ROC - K-NN no Titanic", x = "1 - Especificidade", y = "Sensibilidade") +
  theme_minimal()

# 11. Previsões no conjunto de teste para submissão
test_class_preds <- predict(fit_knn, new_data = test_data) %>%
  bind_cols(test_data)
